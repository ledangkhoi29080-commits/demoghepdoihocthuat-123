...
function NotificationsPage({ currentUser, showToast }: any) {
  const [reports, setReports] = useState<any[]>([]);
  const [myNotifs, setMyNotifs] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

 // Cấp quyền Admin cho tài khoản của bạn (tương thích mọi phiên bản TypeScript)
 const isAdmin = Boolean(
  currentUser?.name === 'Phan Phúc Gia' ||
  currentUser?.email === 'giaphan1801@gmail.com'
);

  const fetchNotifications = async () => {
    setLoading(true);
    try {
      // 1. Nếu là Admin: Tải danh sách tất cả các báo cáo đang chờ xử lý
      if (isAdmin) {
      ...
      // 2. Tải thông báo xác nhận của người dùng này từ LocalStorage
      const localNotifs = JSON.parse(localStorage.getItem(`notifs_${currentUser?.id}`) || '[]');
      setMyNotifs(localNotifs);
    } catch (e) {
      console.warn(e);
    } finally {
      setLoading(false);
  ...
  // Admin xử lý: Trừ điểm uy tín
  const handleAdminAction = async (reportId: string, reportedUserId: string, action: 'penalize' | 'dismiss') => {
    try {
      if (action === 'penalize') {
        // 1. Dùng maybeSingle để tránh ném lỗi nếu profile chưa tồn tại
        const { data: profile } = await supabase
          .from('profiles')
          .select('commitment_rate')
          .eq('id', reportedUserId)
          .maybeSingle();
  
        const currentRate = profile?.commitment_rate !== undefined && profile?.commitment_rate !== null
          ? Number(profile.commitment_rate)
          : 98;
        const newRate = Math.max(30, currentRate - 5);
  
        // 2. Cập nhật hoặc thêm mới nếu profile chưa có
        const { error: updateError } = await supabase
          .from('profiles')
          .upsert({ id: reportedUserId, commitment_rate: newRate });
        ...
        // 3. Đổi trạng thái báo cáo
        await supabase
          .from('reports')
          .update({ status: 'penalized' })
          .eq('id', reportId);
  
        showToast(`Đã duyệt! Điểm uy tín của người dùng bị trừ xuống ${newRate}%`);
      } else {
        await supabase
          .from('reports')
        ...
        showToast('Đã bỏ qua báo cáo này.');
      }

      // Cập nhật lại danh sách trên giao diện
      setReports((prev) =>
        prev.map((r) => (r.id === reportId ? { ...r, status: action === 'penalize' ? 'penalized' : 'dismissed' } : r))
      );
    } catch (err: any) {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '24px auto', padding: '0 16px 40px' }}>
      <div style={{ background: '#fff', borderRadius: 24, padding: 24, boxShadow: '0 8px 30px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          <Bell size={22} color="#3FA796" />
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#223338', margin: 0 }}>Trung tâm Thông báo</h2>
        </div>

        {/* 1. KHU VỰC THÔNG BÁO CÁ NHÂN (USER) */}
        <div style={{ marginBottom: 28 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: '#6B8087', textTransform: 'uppercase', marginBottom: 12 }}>
            Lịch sử hoạt động của bạn
          </h3>
          {myNotifs.length === 0 ? (
            <div style={{ fontSize: 13, color: '#A0AEC0', fontStyle: 'italic', padding: '12px 0' }}>
              Chưa có thông báo nào.
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {myNotifs.map((n: any, idx: number) => (
                <div
                  key={idx}
                  style={{
                    background: '#F4FAF8',
        ...
        {/* 2. KHU VỰC DUYỆT BÁO CÁO (DÀNH CHO ADMIN) */}
        {isAdmin && (
          <div style={{ borderTop: '2px dashed #EDF3F1', paddingTop: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: '#E2574C', textTransform: 'uppercase', margin: 0 }}>
                🛡️ Bảng điều khiển Quản trị viên (Xét duyệt báo cáo)
              </h3>
              <span style={{ fontSize: 11, background: '#FBEAE8', color: '#E2574C', padding: '3px 8px', borderRadius: 99, fontWeight: 600 }}>
                Admin Only
              </span>
            </div>

            {loading ? (
              <div style={{ fontSize: 13, color: '#6B8087' }}>Đang tải danh sách báo cáo...</div>
            ) : reports.length === 0 ? (
              <div style={{ fontSize: 13, color: '#A0AEC0', fontStyle: 'italic' }}>Không có báo cáo nào tồn đọng.</div>
            ) : (
                ...
                {reports.map((r: any) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 14,
                      padding: 16,
                      background: r.status === 'penalized' ? '#FFF5F5' : r.status === 'dismissed' ? '#F8FAFC' : '#FFFFFF',
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ fontWeight: 700, fontSize: 14, color: '#223338' }}>
                        Lý do: <span style={{ color: '#E53E3E' }}>{r.reason}</span>
                      </span>
                    ...
                    <div style={{ fontSize: 13, color: '#4A5568', marginBottom: 10 }}>
                      <strong>Mô tả:</strong> {r.comment || 'Không có ghi chú thêm.'}
                    </div>

                    <div style={{ fontSize: 12, color: '#718096', marginBottom: 12 }}>
                      Mã đối tượng bị báo cáo: <code>{r.reported_user_id}</code>
                    </div>

                    {/* Các nút phê duyệt của Admin */}
                    {r.status === 'penalized' ? (
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#E53E3E' }}>
                        ✓ Đã duyệt phạt (-5% uy tín)
                      </span>
                    ) : r.status === 'dismissed' ? (
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          type="button"
                          onClick={() => handleAdminAction(r.id, r.reported_user_id, 'penalize')}
                          style={{
                            background: '#E2574C',
                            color: '#fff',
                            border: 'none',
                        ...
                        <button
                          type="button"
                          onClick={() => handleAdminAction(r.id, r.reported_user_id, 'dismiss')}
                          style={{
                            background: '#EDF2F7',
                            color: '#4A5568',
                            border: 'none',
...
function ProfilePage({ currentUser, onSave, isReadOnly, onBack, showToast }: any) {
  const [form, setForm] = useState({
    name: currentUser?.name || '',
    major: currentUser?.major || 'Kinh tế đầu tư',
    birthYear: currentUser?.birthYear || currentUser?.birth_year || '2007',
    primarySkill: currentUser?.primarySkill || currentUser?.primary_skill || '',
    experience: currentUser?.experience || currentUser?.bio || '',
    haveText: Array.isArray(currentUser?.skills) ? currentUser.skills.join(', ') : (currentUser?.haveText || currentUser?.skills || ''),
    wantText: Array.isArray(currentUser?.skills_need) ? currentUser.skills_need.join(', ') : (currentUser?.wantText || currentUser?.skills_need || ''),
    goal: currentUser?.goal || currentUser?.project_goal || '',
    avatarUrl: currentUser?.avatarUrl || currentUser?.avatar || null,
    commitmentRate: currentUser?.commitment_rate ?? currentUser?.commitmentRate ?? 98,
  ...
  const handleRemoveProof = async (itemToRemove: any) => {
    const targetId = typeof itemToRemove === 'object' ? itemToRemove?.id : null;
    const targetTitle = typeof itemToRemove === 'object' ? itemToRemove?.title : itemToRemove;

    // Cập nhật giao diện ngay lập tức
    const updated = proofs.filter((p: any) => {
    ...
    // Xóa triệt để khỏi Supabase
    try {
      if (targetId) {
        await supabase.from('skill_proofs').delete().eq('id', targetId);
      } else if (currentUser?.id && targetTitle) {
        await supabase
          .from('skill_proofs')
          .delete()
      ...
      if (currentUser?.id) {
        await supabase
          .from('profiles')
          .update({ proofs: updated, updated_at: new Date().toISOString() })
          .eq('id', currentUser.id);
      }
      if (typeof showToast === 'function') {
        showToast('Đã xóa minh chứng vĩnh viễn!');
      }
    } catch (err) {
      console.warn('Lỗi khi xóa minh chứng khỏi Supabase:', err);
    }
  };
  ...
  // Tự động kéo dữ liệu mới nhất từ Supabase của người đang được xem
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        const src = p || currentUser;
        
        setForm((prev: any) => ({
          ...prev,
          name: src.name || src.full_name || prev.name,
          major: src.major || prev.major,
          birthYear: src.birth_year || src.birthYear || prev.birthYear,
          primarySkill: src.primary_skill || src.primarySkill || prev.primarySkill,
          experience: src.bio || src.experience || prev.experience,
          haveText: Array.isArray(src.skills) ? src.skills.join(', ') : (src.haveText || src.skills || prev.haveText),
          wantText: Array.isArray(src.skills_need) ? src.skills_need.join(', ') : (src.wantText || src.skills_need || prev.wantText),
          goal: src.project_goal || src.goal || prev.goal,
          commitmentRate: src.commitment_rate !== undefined && src.commitment_rate !== null ? Number(src.commitment_rate) : prev.commitmentRate,
          avatarUrl: src.avatar_url || src.avatarUrl || prev.avatarUrl,
        }));

        if (src.proofs && Array.isArray(src.proofs)) {
          setProofs(src.proofs);
        }
      } catch (err) {
        console.warn('Lỗi tải dữ liệu profile:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchCommitment = async () => {
      ...
      if (data && data.commitment_rate !== undefined && data.commitment_rate !== null) {
        setForm((prev: any) => ({ ...prev, commitmentRate: Number(data.commitment_rate) }));
      }
    };
    fetchCommitment();
  }, [currentUser?.id]);
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      // 1. Lưu bản ghi báo cáo vào Supabase (chờ Admin duyệt)
      const { error } = await supabase.from('reports').insert({
        reporter_id: user.id,
        reported_user_id: currentUser.id,
        reason: reportReason,
        comment: reportComment,
        status: 'pending', // Đánh dấu trạng thái chờ Admin xử lý
      });

      if (!error) {
        // 2. Tạo thông báo lưu vào lịch sử của chính người gửi báo cáo
        const targetName = currentUser?.name || 'Người dùng';
        const today = new Date().toLocaleDateString('vi-VN');
        const notifMessage = `Bạn đã báo cáo người dùng "${targetName}" vào ngày ${today}. Dữ liệu đã được ghi nhận và gửi đến hệ thống quản trị.`;

        try {
          const notifKey = `notifs_${user.id}`;
          const prevNotifs = JSON.parse(localStorage.getItem(notifKey) || '[]');
          localStorage.setItem(
            notifKey,
            JSON.stringify([{ content: notifMessage, date: today, id: Date.now() }, ...prevNotifs])
          );
        } catch (storageErr) {
          console.warn('Lỗi lưu thông báo local:', storageErr);
        }

        showToast('Đã gửi báo cáo! Bạn có thể xem lại trong mục Thông báo.');
      ...
      showToast('Lỗi kết nối: ' + (err.message || 'Không thể gửi'));
    } finally {
      setIsSubmittingReport(false);
    }
  };
...
{((form as any).avatar || (form as any).avatar_url || form.avatarUrl) ? (
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          onClick={() => {
            if (!isReadOnly) {
              setReportReason('Tự đánh giá / Ghi chú bản thân');
            } else {
              setReportReason('Rời nhóm giữa chừng');
            }
            setShowReportModal(true);
          }}
          style={{
            background: 'transparent',
            border: isReadOnly ? '1px solid #FFCDD2' : '1px solid #DCEAE6',
            color: isReadOnly ? '#D32F2F' : '#3FA796',
            padding: '4px 12px',
            borderRadius: 8,
            fontSize: 12,
            fontWeight: 600,
        ...
        >
          {isReadOnly ? '🚩 Báo cáo / Đánh giá đối tác' : '📝 Tự đánh giá bản thân'}
        </button>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
          {(!Array.isArray(proofs) || proofs.length === 0) ? (
            ...
            proofs.map((item: any, idx: number) => {
              const title = typeof item === 'string' ? item : (item?.title || item?.skill_name || item?.name || 'Minh chứng');
              const rawUrl = typeof item === 'string' 
                ? (item.startsWith('http') || item.startsWith('data:') ? item : '') 
                : (item?.url || item?.image_url || item?.link || item?.file_url || '');
              ...
              const handleClickProof = (e: React.MouseEvent) => {
                e.stopPropagation();
                if (!rawUrl) {
                  showToast('Minh chứng này chưa có đường dẫn hoặc tệp đính kèm!');
                  return;
                }
                if (isBase64 || rawUrl.match(/\.(jpeg|jpg|gif|png|webp)/i)) {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    cursor: rawUrl ? 'pointer' : 'default',
                    marginBottom: 6,
                  }}
                  title={rawUrl ? 'Bấm để xem ảnh hoặc mở liên kết' : ''}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden', flex: 1 }}>
                    ...
                    {linkHref && (
                      ...
                      >
                        {isBase64 || rawUrl.match(/\.(jpeg|jpg|gif|png|webp)/i) ? '(Xem ảnh ↗)' : '(Mở Drive ↗)'}
                      </span>
                    )}
                  </div>
 ...
 {uploadingSkill === skill && (
  <span style={{ fontSize: 11, color: '#0284C7', fontStyle: 'italic' }}>⏳ Đang tải...</span>
)}
                  </div>
                );
              })}
...
export default function App() {
  ...
  const handleSaveProfile = async (formData: any) => {
    ...
    if (currentUser || formData?.id) {
      try {
        const payload: any = {
          id: currentUser?.id || formData?.id,
          full_name: formData.name || '',
          major: formData.major || '',
          birth_year: formData.birthYear || '2007',
          primary_skill: formData.primarySkill || '',
          bio: formData.experience || '',
          skills: typeof formData.haveText === 'string'
            ? formData.haveText.split(',').map((s: string) => s.trim()).filter(Boolean)
            : (formData.have || []),
          skills_need: typeof formData.wantText === 'string'
            ? formData.wantText.split(',').map((s: string) => s.trim()).filter(Boolean)
            : (formData.want || []),
          project_goal: formData.goal || '',
        ...
        if (error) {
          ...
          if (typeof showToast === 'function') {
            showToast('Lỗi lưu cơ sở dữ liệu: ' + error.message);
          }
          return;
        }

        if (typeof showToast === 'function') {
          showToast('Đã lưu hồ sơ và đồng bộ lên hệ thống thành công!');
        }
      } catch (err: any) {
        console.error('Lỗi kết nối:', err);
        if (typeof showToast === 'function') {
          showToast('Lỗi mạng hoặc kết nối cơ sở dữ liệu!');
        }
        return;
      }
    }
  ...
  return (
    ...
    >
      ...
      {page === 'notifications' && isLoggedIn && (
        <NotificationsPage currentUser={currentUser} showToast={showToast} />
      )}
      {(page === 'swipe' || page === 'chat' || page === 'profile' || page === 'notifications') &&
